...
def plot_x_dividers(axis, chrom_sizes, pad=None):
    ...
    for xposn in x_dividers[:-1]:
        axis.axvline(x=xposn, color='k', linewidth=0.5)
...
def cvg2rgb(cvg, desaturate):
    ...
    cutoff = 6  # Values above this magnitude are shown with max intensity
...
#        rgb = (1 - .25*x, 1 - s, 1 - s)  # Reddish
    ...
    if cvg < 2:
        if cvg==0:
            ...
            s=cvg/2
        rgb = (0 + s, 0 + s, 1)  # Blueish
    else:
        if cvg==2:
            rgb = (1, 1, 1)  # white
        elif cvg<=4:
            s=min((cvg-2)/2,1)
            rgb = (1, 1 - s, 1 - s)  # Reddish
        else:
            s=min(cvg-4,2)
            s=s/4
            rgb = (1-s,0,0) # Brownish
    return rgb
